...
$(document).ready(function() {
	...
	if (!sessionStorage.mobileops_email_address) {
		...
		if (getUrlParameter('editbooking')) {
			...
			getClientProfile();
			var d = new Date();
			intialiseDateTimePicker(d.getDay() + '.' + d.getMonth() + '.' + d,getYear(), '09:00');
		}
...
function getBookingDetails(){
	...
	$.ajax({
		type : 'GET',
		url : 'src/controller/controller_bookingdetails.php?getBookingDetails=' + getUrlParameter("editbooking"),
		...
		success : function(data) {
			...
			$('#input_city').val(data['sublocality']);
			$('#input_suburb').val(data['locality']);
...
function getAllServices(){
	$.ajax({
		...
		$.getScript( "js/out/jquery.idealforms.js", function( data, textStatus, jqxhr ) {
			  	    ...
			  	    if (invalid) {
			  	      ...
			  	      if(("Step 5".localeCompare(selectedStep[0].firstChild.firstChild.data) == 0) ){
			  	    	  if("Complete Booking".localeCompare(e.originalEvent.explicitOriginalTarget.innerHTML) == 0){
			  	    		 $.post('src/controller/controller_booking.php?completeBooking=true', this.$form.serialize(), function(response) {  
...
function getTotalAmountDue(formdata){
	...
	$.ajax({
		type : 'GET',
		url : 'src/controller/controller_booking.php?getServicePrices=true&region=' + $('#input_suburb').val(),
...
function getBestPartners(formdata){
	$("#bestPartnersDiv").load("src/controller/controller_booking.php?getBestPartners=getBestPartners&" + formdata, function() {
