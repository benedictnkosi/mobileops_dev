...
$(document).ready(function() {
	   ...
	   if (!sessionStorage.mobileops_email_address) {
	    ...
	    $.ajax({
	        url:'images/partner_profile/profile_' +sessionStorage["mobileops_user_id"] + '.jpg',
	        type:'HEAD',
	        error: function()
	        {
	    	document.getElementById("upload-file-container").style.background = "url(images/partner_profile/noimage.jpg) no-repeat";
	        },
	        success: function()
	        {
	        	document.getElementById("upload-file-container").style.background = "url(images/partner_profile/profile_" +sessionStorage["mobileops_user_id"] + ".jpg) no-repeat";
	        }
	    });
	    

	    var thumb = $('#upload-file-container');        
	    ...
	    new AjaxUpload('imageUpload', {
	      ...
	      onComplete: function(file, response) {
	    	  d = new Date();
	    	  $('#upload-file-container').css("background-image", "url(images/partner_profile/profile_" +sessionStorage["mobileops_user_id"] + ".jpg?" + d.getTime() + ") no-repeat");
	    	  location.reload();
	      }
	    });
    ...
    $("#fileUpload_dropzone").dropzone({
        ...
        init: function() {
            thisDropzone = this;
            $.get('/src/Logic/file_upload.php', function(data) {
            ...
            this.on("removedfile", function(file) {
            	...
            	for (i = 0; i < mobileops_uploadedImages.length; i++) { 
            		...
            		if(uploadedImage[1].localeCompare(file.name) == 0){
            			...
            			sessionStorage["mobileops_uploadedImages"] = JSON.stringify(mobileops_uploadedImages);
            			$.get("src/controller/controller_partner_profile.php?deleteimage=thumb_" + uploadedImage[0], function(data,
                                status) {
				...
				// the server
            	if(!isFileDeleted){
            		$.get("src/controller/controller_partner_profile.php?deleteimage=" + file.name, function(data,
                            status) {
                            if (data.indexOf("success") > -1) {
                            	return;
                            } else {
...
function getPartnerProfile() {
    $.ajax({
        type: 'GET',
        url: 'src/controller/controller_partner_profile.php',
