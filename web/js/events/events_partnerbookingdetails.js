...
$(document).ready(function() {
	...
	$('#acceptBooking').click(function(event){
...
function acceptBooking(){
	var r = confirm("Are you sure you want to accept this booking?");
	if (r == false) {
	    return;
	} 
	...
	$.ajax({
		type : 'POST',
		url : 'src/AppBundle/Controller/controller_booking.php',
		 data: {"acceptBooking" : getUrlParameter("partnerbookingdetails"), "uuid":getUrlParameter("uuid")}, 
		 dataType : "json",
		success : function(response) {
			$('.loading').remove();
			var message = response.message;
			if(response.status ==  1){
				...
				element.appendChild(document.createTextNode("Active"));
				
				$('#acceptBooking').addClass('display-none')
...
function getBookingDetails(){
	...
	$.ajax({
		type : 'GET',
		url : 'src/AppBundle/Controller/controller_booking.php?getBookingDetails=' + getUrlParameter("partnerbookingdetails") + "&uuid=" + getUrlParameter("uuid"),
		data : 'bookingId=' + getUrlParameter("partnerbookingdetails"),
		dataType : "json",
