...
class FB_Login {
	...
	*/
	...
	public function __construct($entityManager) {
		...
		elseif (isset ( $_POST ["loadFacebookUser"] )) {
	...
	*/
	...
	private function dologinWithPostData($entityManager) {
		...
		try {
			...
			//echo $_POST ['fb_userid'];
			$user =  null;

			//get user instance using FB user id. only the fb id is required to authenticate
			$userByFBId = $entityManager->getRepository('User')->findOneBy(array('facebookNetworkId' => $_POST ['fb_userid']));

			//check if email already registered
			$userByEmail = $entityManager->getRepository('User')->findOneBy(array('emailAddress' => $_POST ['loadFacebookUser']));
			...
			if($userByFBId){
				...
				//save the FacebookNetworkId to user
				$user->setFacebookNetworkId($_POST ['fb_userid']);
			...
			'user_id'=>$_SESSION ['user_id'],
				...
				$UserProfile->setFirstName($_POST ['first_name']);

				$UserProfile->setSurname($_POST ['last_name']);
				...
				$user->setEmailAddress($_POST ['loadFacebookUser']);
